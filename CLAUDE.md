...
- `src/cartlog/parsing/` — Pydantic AI vision parser, category classifier, a focused LLM size extractor (`size_extractor.py`, `build_size_extractor`) that recovers a package size from line text, and `structuring.py` (see below).
- `src/cartlog/parsing/structuring.py` — `structure_line` turns a line's raw text + quantity into a `StructuredMeasure` (its `sold_by` mode plus `measure_unit`/`size_amount`/`size_unit` and provenance). Home of the deterministic free-text recovery: `parse_size`, `extract_size`, `repair_size_token` (relocated from `units.py`), and the `detect_count_sale` / `detect_measure_sale` sale-basis detectors. Consumed by ingest, the backfill, and the size sweep.
- `src/cartlog/units.py` — pure measure arithmetic: `compute_measure` turns the structured fields into the normalized base measure and price, plus the `SoldBy`/`MeasureSource`/`MeasureStatus` enums and `normalize_unit_token`. All free-text recovery lives in `parsing/structuring.py`.
- `src/cartlog/sizes/` — `extract.py` runs the LLM size extractor over `SoldBy.ITEM` lines that still lack a `size_amount` (MEASURE lines need no per-package size; MANUAL lines are skipped), capped per line by `CARTLOG_MAX_SIZE_EXTRACT_ATTEMPTS`.
- `src/cartlog/db/` — SQLAlchemy models, session factory, and seed data. `backfill.py` holds the four-pass `normalize_existing_measures` (deterministic structure+resolve, LLM size recovery, typical-size learning, inference) and runs at startup, writing the structured measure columns via `_apply_structured`; pass 1 skips lines whose `measure_source` is `MANUAL` (human-pinned) or `INFERRED` (owned by pass 4). `apply_line_item_edit` (`receipts/service.py`, the search inline editor) pins `MANUAL` when a human edits a line's sold-by mode, measure unit, or size, so the backfill never overwrites that edit. Migrations live in the top-level `alembic/`.
...
- Run the test suite with pytest. It runs in parallel across all cores by default via pytest-xdist (`-n auto` is in `addopts`); pass `-n0` to force serial execution when debugging a single test with `-s` or a breakpoint:
    ```bash
    uv run pytest
    uv run pytest -n0 -s tests/path/to/test.py   # serial, for debugging
    ```
- Password hashing is stubbed with a low-cost Argon2 hasher in tests via an autouse fixture; a test that must assert the production Argon2id parameters opts out with `@pytest.mark.real_hashing`.
